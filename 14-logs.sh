...
if [ $? -ne 0 ]; then
    ...
    echo -e " $Y MariaDB is already installed $N .. nothing to do" | tee -a $LOG_FILE
...
if [ $? -ne 0 ]; then
    ...
    echo -e "nothing to do..$Y nginx is already installed $N" | tee -a $LOG_FILE
...
#tee -a $LOG_FILE will show to user and store in log file

#&>>$LOG_FILE only will store in logfile, will not show on screen

# sudo sh 14-logs.sh 
# [ec2-user@ip-172-31-18-115 Shell-Practice]$ cat /var/log/shellscript-logs/14-logs.log 